...
def show_jira_settings_panel(config, save_callback=None):
    ...
    with st.form("jira_settings_form"):
        ...
        **🔑 API Token Required:**
        ...
        3. Copy the token and save it in your `.env` file as `JIRA_API_TOKEN=your_token_here`
        4. Never share your API token publicly!
        """)
        ...
        for internal_status, jira_status in status_mappings.items():
            ...
            with cols[1]:
                new_mappings[internal_status] = st.text_input(
                    f"Maps to",
                    value=jira_status,
